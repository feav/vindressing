...
<body>
	...
	<?php
			...
			?>
			<H1><div class="round-count"><?php echo $count; ?></div> Gestion des annonces expirer</H1>
			<style>
			.englobehelp
			{
				display:initial;
			...
			<div class="info">
			Gérer les annonces expirer de votre site internet. Les annonces expirer apparaisse en fonction de vos réglages de durée de validité d'une annonce et en fonction
			de l'activation ou non de la tache cron permettant leur expiration. Les annonces expirer ne sont plus visible sur le site internet et ne peuvent être réactiver par l'utilisateur que par le renouvellement de l'annonce ou manuellement en la modifiant depuis cette page.
			</div>
			<?php
			...
			if(isset($_REQUEST['search']))
			{
				...
				$addSearch = "WHERE titre LIKE '%$search%' AND valider = 'expired' AND status != 'NOVISIBLE'";
			}
			else
			{
				$addSearch = "WHERE status != 'NOVISIBLE' AND valider = 'expired'";
			}
			...
			<table>
				...
				if($count_expired == 0)
				{
					...
					<h1>Aucune annonce expirée pour le moment</h1>
					</div>
					<?php
				}
				else
				{
					...
					$SQL = "SELECT COUNT(*) FROM pas_annonce $addSearch AND valider = 'expired'";
					$reponse = $pdo->query($SQL);
					$req = $reponse->fetch();
					...
					$SQL = "SELECT * FROM pas_annonce $addSearch AND valider = 'expired' ORDER BY date_soumission DESC LIMIT $totalp,$nombre_annonce";
					$reponse = $pdo->query($SQL);
