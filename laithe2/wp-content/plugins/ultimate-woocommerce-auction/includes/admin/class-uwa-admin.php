	...
	*/
     public function uwa_options_product_tab_content() {
        global $post;
			...
			$timezone = get_option('timezone_string') ? get_option('timezone_string') : 
				"UTC";
	...
	*/
    function uwa_save_auction_option_field( $post_id ) {
		...
		if ( $product_type == 'auction' ) {
			...
			$auction_type = "normal";
			update_post_meta( $post_id, 'woo_ua_auction_type', 
				esc_attr( $auction_type ));


			 /* Save starttime only when new auction is added  */
