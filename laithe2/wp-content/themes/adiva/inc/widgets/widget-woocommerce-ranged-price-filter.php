...
if(function_exists('is_woocommerce')) {
	class Adiva_WC_Widget_Ranged_Price_Filter extends WC_Widget {
		...
		public function __construct() {
			...
			$this->widget_description = esc_html__( 'Shows a price filter list in a widget which lets you narrow down the list of shown products when viewing product categories.', 'adiva' );
			$this->widget_id          = 'adiva_woocommerce_ranged_price_filter';
			$this->widget_name        = esc_html__( 'JMS - WC Ranged Price Filter List', 'adiva' );
			$this->settings           = array(
				'title' => array(
					'type'  => 'text',
					'std'   => esc_html__( 'Filter by price', 'adiva' ),
				...
				'price_range_size' => array(
					...
					'max'   => '',
					'std'   => 100,
					'label' => esc_html__( 'Range size', 'adiva' )
				),
				'max_price_ranges' => array(
					...
					'max'   => '',
					'std'   => 5,
					'label' => esc_html__( 'Max number of ranges', 'adiva' )
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			<?php
			if ( $min_price  >= $min ) {
				echo '<li><a href="' . esc_url( $link ) . '">' . esc_html__( 'All', 'adiva' ) . '</a></li>';
			} else {
	            echo '<li class="current">' . esc_html__( 'All', 'adiva' ) . '</li>';
			}
			...
			for ( $range_min = $min; $range_min < ( $max + $range_size ); $range_min += $range_size ) {
				...
				if ( $count == $max_ranges ) {
					...
					if ( $range_min != $min_price ) {
						...
						echo '<li class="current">' . $price_output . '</li>';
					}
	...
	add_action( 'widgets_init', 'adiva_woocommerce_ranged_price_widget', 25 );

	function adiva_woocommerce_ranged_price_widget() {
	    register_widget( 'Adiva_WC_Widget_Ranged_Price_Filter' );
	}
}
