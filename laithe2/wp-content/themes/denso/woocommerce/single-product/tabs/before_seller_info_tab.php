<h3 class="widget-title"><?php esc_html_e( 'Seller Info', 'denso' ); ?></h3>
<div class="row">
	<?php
				...
				<div class="media-left">
					<div class="store-brand">
						<a href="<?php echo esc_url($store_url); ?>">
							<img src="<?php echo esc_url($store_icon); ?>" alt="" class="store-icon" />
						</a>
					</div>
				</div>
				<?php } ?>
					...
					$ratings = '';
		  			if ( class_exists('WCVendors_Pro') && !WCVendors_Pro::get_option( 'ratings_management_cap' ) ) {
		  				...
		  				<div class="seller-info-rating">
			  				...
			  				if ( $average_rate !=0 ) {
				  				echo sprintf(_n('Rating: <strong>%s</strong> based on %d rating', 'Rating: %s based on %d ratings', $rate_count, 'denso'), $average_rate, $rate_count);
				  				echo '<a href="'.esc_url($url).'" class="btn">'.esc_html__('View all ratings', 'denso').'</a>';
			  				} else {
			  					echo esc_html__("Rating: This Seller still doesn't have any ratings yet.", 'denso');
			  				}
			  				?>
		  				</div>
		  				<?php
