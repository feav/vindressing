...
if ( !class_exists("Denso_Nav_Menu") ) {
    class Denso_Nav_Menu extends Walker_Nav_Menu {
       ...
       /**
        ...
        public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
            ...
            $class_names .= $apus_mega_profile ?' has-sub ':'';
            ...
            if ( isset($item->apus_text_label) && !empty($item->apus_text_label) ) {
                switch ( $item->apus_text_label ) {
                    case 'label_new':
                        $text_label  = esc_html__('New', 'denso');
                        break;

                    case 'label_hot':
                        $text_label  = esc_html__('Hot', 'denso');
                        break;

                    case 'label_featured':
                        $text_label  = esc_html__('Featured', 'denso');
                        break;
...
// icon hover
function denso_custom_megamenu($item) {
    $item_id = esc_attr( $item->post_name );
    $highlights = array(
        '' => esc_html__('No', 'denso'),
        'highlight' => esc_html__('Yes', 'denso'),
    );
    ?>
    <p class="field-apus_highlight description description-wide">   
        <label for="edit-menu-item-apus_highlight-<?php echo esc_attr($item_id); ?>"><?php esc_html_e( 'Highlight:', 'denso' ); ?> <br>
            <select name="menu-item-apus_highlight[<?php echo esc_attr($item_id); ?>]">
                <?php foreach( $highlights as $key => $align ) { ?>
                <option <?php selected( esc_attr($item->apus_highlight), $key ); ?> value="<?php echo esc_attr($key); ?>"><?php echo esc_html($align); ?></option>
                <?php } ?>
            </select>
        </label>
    </p>
...
add_action( 'apus-themer-megamenu-after-icon', 'denso_custom_megamenu' );

function denso_custom_nav_item($menu_item) {
    $fields = array( 'apus_highlight' );
...
add_filter( 'wp_setup_nav_menu_item', 'denso_custom_nav_item' );

function denso_custom_nav_update($menu_id, $menu_item_db_id, $args ) {
...
add_action( 'wp_update_nav_menu_item', 'denso_custom_nav_update', 10, 3 );