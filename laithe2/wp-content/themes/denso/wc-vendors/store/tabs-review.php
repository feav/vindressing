<div id="vendor-ratings" class="tab-pane active">
	...
	<?php
		...
		$detail_ratings = denso_get_vendor_detail_ratings( $vendor_id );
	...
	if ( $vendor_ratings ) {
			...
			<div class="vendor-reviews-info col-xs-12 col-sm-3">
				<div class="vendor-reviews-inner">
					<div class="average-value">
						<?php echo sprintf(__( '%s <span>overall</span>', 'denso' ), $average_rate); ?>
					</div>
					<div class="title-info">
						<?php echo sprintf(_n('Based on %d review', 'Based on %d reviews', $rate_count, 'denso'), $rate_count); ?>
					</div>
							...
							<div class="detailed-rating-inner">
								...
								<?php for ( $i = 5; $i >= 1; $i -- ) : ?>
									<div class="skill special-progress">
										...
										<div class="progress">
											<div class="value-percent hidden"><?php echo ( $rate_count && !empty( $detail_ratings[$i] ) ) ? esc_attr(  round( $detail_ratings[$i] / $rate_count * 100, 2 ) . '%' ) : '0%'; ?></div>
											<div class="progress-bar progress-bar-default" style="<?php echo ( $rate_count && !empty( $detail_ratings[$i] ) ) ? esc_attr( 'width: ' . ( $detail_ratings[$i] / $rate_count * 100 ) . '%' ) : 'width: 0%'; ?>">
											</div>
										</div>
					...
					<span><?php esc_html_e('Latest reviews presented here ', 'denso'); ?></span>
					<p class="view-rating"><a class="btn btn-theme btn-outline" href="<?php echo esc_url($reviews_url); ?>"><?php esc_html_e('View all ratings', 'denso'); ?></a></p>
				</div>
			</div>
			<?php
		}
				...
				<div class="single-rating">
					<div class="media">
						...
						<div class="media-body">
							...
							<h4><span class="name"><?php echo esc_attr($customer_name); ?> </span><span><?php esc_html_e( '-', 'denso'); echo esc_attr($post_date); ?></span></h4>
		...
		<?php
	} else {  esc_html_e( 'No ratings have been submitted for this vendor yet.', 'denso' ); }  ?>
	</div>
</div>